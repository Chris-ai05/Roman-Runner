...
export class Obstacles {
  ...
  private bannerPoleGeo = new THREE.CylinderGeometry(0.08, 0.08, 2.4, 6);
  ...
  private spawnBanner(z: number, lane: number) {
    ...
    // Banner (das eigentliche Hindernis - hängt so tief, dass der stehende
    // Spieler klar trifft, der geduckte aber drunter durchpasst)
    const cloth = new THREE.Mesh(this.bannerClothGeo, this.bannerClothMat);
    cloth.position.set(0, 1.6, 0);
    group.add(cloth);

    // Querbalken oben (über dem Tuch)
    ...
    // Hitbox: Banner-Tuch (1.6 breit, 1.2 hoch, Center y=1.6) -> y=1.0 bis y=2.2
    // Stehender Spieler ist ~1.71 hoch (trifft sicher), geduckt ~0.94 (passt drunter).
    const hb = new THREE.Box3(
      new THREE.Vector3(LANE_X[lane] - 0.8, 1.0, z - 0.2),
      new THREE.Vector3(LANE_X[lane] + 0.8, 2.2, z + 0.2)
    );

 ...
 *   │  └─ tunic, cape, gürtel, etc. (statisch)
...
export class Player {
  ...
  private buildFigure() {
    ...
    // Tunika-Saum unten - dünner Creme-Ring
    const hemGeo = new THREE.TorusGeometry(s * 0.55, s * 0.04, 6, 16);
    const hem = new THREE.Mesh(hemGeo, tunicTrimMat);
    hem.position.y = -s * 0.4;
    ...
    // === Cape ===
    const capeGeo = new THREE.PlaneGeometry(s * 1.1, s * 1.2, 4, 4);
    this.cape = new THREE.Mesh(capeGeo, capeMat);
    this.cape.position.set(0, s * 0.3, -s * 0.32);
    this.cape.rotation.x = 0.15;
    this.torso.add(this.cape);

    // Brosche (Cape-Schließe)
    const broochGeo = new THREE.SphereGeometry(s * 0.08, 10, 8);
    const brooch = new THREE.Mesh(broochGeo, goldMat);
    brooch.position.set(s * 0.32, s * 0.82, -s * 0.05);
    this.torso.add(brooch);
  ...
  update(dt: number, runTime: number) {
    ...
    // Cape flattert leicht beim Laufen
    this.cape.rotation.x = 0.15 + Math.sin(runTime * 9) * 0.04;
